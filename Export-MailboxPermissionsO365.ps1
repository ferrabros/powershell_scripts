...
$UseImportFile = $false
$ImportFile = "c:\_scripts\UserList.csv"
...
ForEach ($Mailbox in $Mailboxes) {
    ...
    If ($IncludeSendAs -eq $true) {
        ...
        $Delegates = @()
        $Delegates = Get-RecipientPermission $MailboxDN | Where { $DelegatesToSkip -notcontains $_.User -and $_.ExtendedRights -like "*send-as*" }
        If ($Delegates -ne $null) {
            ForEach ($Delegate in $Delegates) {
